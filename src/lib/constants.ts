export const CONSTANTS = { SITE_TITLE: "YTHWKND",
  SITE_SUBTITLE: "AND THE",
  SITE_MAIN_TITLE: "MULTIVERSE OF MYSTERY",
  SITE_DESCRIPTION: "A HIGHSCHOOL EVENT BY @YMFGAKL",

  // Class options for party registration
  HEROES: [
    {
      id: "warrior",
      name: "Warrior",
      icon: "/warrior.png",
      class: "WARRIOR",
      perk: "FRONTLINE BREAKER",
      description:
        "Frontline Breaker. Smashes through obstacles, absorbs the hardest hits, and charges first when the mission turns brutal.",
    },
    {
      id: "archer",
      name: "Archer",
      icon: "/archer.png",
      class: "ARCHER",
      perk: "PRECISION STRIKER",
      description:
        "Precision Striker. Reads the battlefield, finds the opening, and delivers the shot that changes everything—right when time runs out.",
    },
    {
      id: "scout",
      name: "Scout",
      icon: "/scout.png",
      class: "SCOUT",
      perk: "RAPID PATHFINDER",
      description:
        "Rapid Pathfinder. Moves fast, gathers intel, and slips through the cracks to turn impossible routes into winning plays.",
    },
    {
      id: "guardian",
      name: "Guardian",
      icon: "/guardian.png",
      class: "GUARDIAN",
      perk: "IRON WALL",
      description:
        "Iron Wall. Holds the line, shields the team, and refuses to let chaos break the formation.",
    },
    {
      id: "scholar",
      name: "Scholar",
      icon: "/scholar.png",
      class: "SCHOLAR",
      perk: "TACTICAL MIND",
      description:
        "Tactical Mind. Deciphers patterns, solves locks, and builds the plan that unlocks victory when strength alone won't.",
    },
  ],

  // Party options (20 color-based parties, 5 slots each)
  TEAMS: [
    { id: 1, name: "Crimson", color: "bg-team-01", code: "P001" },
    { id: 2, name: "Sapphire", color: "bg-team-02", code: "P002" },
    { id: 3, name: "Emerald", color: "bg-team-03", code: "P003" },
    { id: 4, name: "Amber", color: "bg-team-04", code: "P004" },
    { id: 5, name: "Violet", color: "bg-team-05", code: "P005" },
    { id: 6, name: "Cobalt", color: "bg-team-06", code: "P006" },
    { id: 7, name: "Jade", color: "bg-team-07", code: "P007" },
    { id: 8, name: "Coral", color: "bg-team-08", code: "P008" },
    { id: 9, name: "Slate", color: "bg-team-09", code: "P009" },
    { id: 10, name: "Gold", color: "bg-team-10", code: "P010" },
    { id: 11, name: "Ivory", color: "bg-team-11", code: "P011" },
    { id: 12, name: "Onyx", color: "bg-team-12", code: "P012" },
    { id: 13, name: "Rust", color: "bg-team-13", code: "P013" },
    { id: 14, name: "Teal", color: "bg-team-14", code: "P014" },
    { id: 15, name: "Bronze", color: "bg-team-15", code: "P015" },
    { id: 16, name: "Silver", color: "bg-team-16", code: "P016" },
    { id: 17, name: "Scarlet", color: "bg-team-17", code: "P017" },
    { id: 18, name: "Azure", color: "bg-team-18", code: "P018" },
    { id: 19, name: "Maroon", color: "bg-team-19", code: "P019" },
    { id: 20, name: "Indigo", color: "bg-team-20", code: "P020" },
  ],

  // Class image paths — placeholder for now, classes share a single icon per class
  HERO_IMAGE_PATHS: [] as { teamId: number; hero: string; path: string }[],

  // Updated CG Leaders for 2026
  CG_LEADERS: [
    "Annabelle Foo",
    "Axel Chong",
    "Christopher + Oscar",
    "Daniel Loo",
    "David Tiah",
    "Fanna Tan",
    "Japheth",
    "Jenisha Kong",
    "Jessica Lee",
    "Jonathan Yee",
    "Joshua Yap",
    "Kujeyna",
    "Le Shiuan",
    "Rachel Anne",
    "Samantha Edwin",
    "Yae-ber Neo",
    "Not Sure",
  ],

  // Mock ages for demo
  MOCK_AGES: [13, 14, 15, 16, 17],
};
